...
RUN lake build Postgres Html Htmx Routing Middleware MiddlewareCookieStore AwsLambdaHttp \
      AuthenticationOidc AuthenticationOidcHttp AuthenticationFetch
...
# it. `lean-libcrypto`'s shim links `libcrypto` directly, but the base image already carries it:
# asking for `openssl-libs` by name fails, because what satisfies it here is
...
# leancurl and lean-libcrypto each build a shim as a shared library, and the binary loads them by
# name at startup, so the binary alone is not a working function. `/usr/lib64` rather than
# alongside the binary, because it is searched without depending on what the runtime sets
# `LD_LIBRARY_PATH` to.
COPY --from=build /src/.lake/packages/leancurl/.lake/build/lib/libleancurl_shim.so /usr/lib64/
COPY --from=build /src/.lake/packages/lean-libcrypto/.lake/build/lib/libcrypto_shim.so /usr/lib64/
